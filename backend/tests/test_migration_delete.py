...
def can_delete_job(status: str) -> bool:
    return status not in ("in_progress", "discovering", "paused") and status in DELETABLE_JOB_STATUSES
...
def test_delete_blocked_while_paused():
    assert not can_delete_job("paused")
